...
public class VentanaMascotas extends JFrame {
    private JTextField txtId, txtNombre, txtEspecie, txtRaza, txtDocumentoDueno;
    ...
    public VentanaMascotas() {
        ...
        txtDocumentoDueno.setBounds(120, 180, 200, 25);
        ...
        btnRegistrar.addActionListener(e -> {
            String docDueno = txtDocumentoDueno.getText().trim();
            if (docDueno.isEmpty()) {
                txtResultado.setText("Por favor ingrese el documento del dueño.");
                return;
            }

            PersonaVO dueno = personaDAO.consultar(docDueno);
            if (dueno == null) {
                txtResultado.setText("Dueño no registrado.");
                return;
            }

            MascotaVO m = new MascotaVO(txtNombre.getText(), txtEspecie.getText(), txtRaza.getText(), docDueno);
            if (dao.insertar(m)) {
        ...
        btnConsultar.addActionListener(e -> {
            try {
                ...
                MascotaVO m = dao.consultar(id);
                if (m != null) {
                    txtNombre.setText(m.getNombre());
                    txtEspecie.setText(m.getEspecie());
                    txtRaza.setText(m.getRaza());
                    txtDocumentoDueno.setText(m.getDocumentoDueno());
                    PersonaVO dueno = personaDAO.consultar(m.getDocumentoDueno());
        ...
        btnActualizar.addActionListener(e -> {
            try {
                int id = Integer.parseInt(txtId.getText());
                MascotaVO m = new MascotaVO(id, txtNombre.getText(), txtEspecie.getText(), txtRaza.getText(), txtDocumentoDueno.getText());
                if (dao.actualizar(m)) {
